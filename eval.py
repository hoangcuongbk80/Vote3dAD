...
def eval(cfgs):
    global cfg
    cfg = cfgs
    from network.Vote3D import VoteAD as net
    from network.Vote3D import eval_fn
    ...
    if cfg.dataset == 'AnomalyShapeNet':
        ...
        tag = 'positive'
    elif cfg.dataset == 'Vote3D':
        from datasets.Vote3D.dataset_preprocess import Dataset
        gt_mask_path = f'datasets/Vote3D/Vote3D-AD/{cfg.category}/gt/'
        tag = 'good'
    else:
